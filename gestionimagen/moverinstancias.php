...
$serie_fk = $_REQUEST['serie_fk'];
 
foreach($_SESSION['instanciamover'] as $instancia_pk) {

  $query = $cnnpacs->prepare("UPDATE instance SET series_fk = ? WHERE pk = ?");
  // $query = $cnnpacs->prepare("UPDATE series SET study_fk = ? WHERE pk = ?");
  $query->execute(array($serie_fk, $instancia_pk));
