...
$_SESSION['instanciamover'] = array();

$listainstancias = $pacss->ListaInstancias($_REQUEST['series_pk']);?>
<?php      while ($reginstancias = $listainstancias->fetch()) {
      
      ?>
         <?php if($reginstancias['ocultoinstance_id']==null){ //No esta oculta ?>
                 <li class="list-group-item">
                    <a  href="#"><?php// echo $reginstancias['inst_no'] ?>
                    <button class="btn btn-success btn-sm" onclick="verInstancia('<?php echo $study_iuid ?>','<?php echo $series_iuid ?>','<?php echo $reginstancias['sopiuid'] ?>')"><i class="fa fa-file-image" title="Ver Imagen"></i></button></a>&nbsp;&nbsp;
                    <button class="btn btn-success btn-sm" onclick="ocultarInstancia('<?php echo $reginstancias['sopiuid'] ?>')"><i class="fa fa-thumbs-down" title="Ocultar Instancia"></i></button>&nbsp;&nbsp;
                    <?php if($_SESSION['usuario_eliminaestudio']==1){ ?><button class="btn btn-danger btn-sm" onclick="eliminarInstancia('<?php echo $reginstancias['sopiuid'] ?>')"><i class="fa fa-trash" title="Eliminar Instancia"></i></button><?php } ?>
                    <input type="checkbox" id="instancia<?php echo $reginstancias['pk'] ?>" onclick="agregarInstancia(<?php echo $reginstancias['pk'] ?>, $(this).prop('checked'));">
                  </li>
...
<script>
  ...
  function agregarInstancia(instancia_pk, agregar){ //agrega la instancia que se sera movida a un arreglo
    $.ajax ({
      type: "POST",
      url: "gestionimagen/agregarinstancia.php",
      data: "instancia_pk="+instancia_pk+"&agregar="+agregar, 
